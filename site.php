   ...
   $open_cle = fopen("../uploads/Produits.csv","r");
    ...
    while ( ($data = fgetcsv($open,2000,";")) == true && $i>=0  ){
        ...
        if($i>=2){ 
            ...
            $departement = $data[42];
            // On vÃƒÂ©rifie si le departement est deja present dans le menu
        ...
        <tr class="header">
                ...
                <?php  
                        ...
                        function tableau($data,$numero){
                            ...
                            */
                            if(count($data[3]) == 1){
                                echo "<tr>";
                                echo "<td><a style='text-decoration:none;color:black;' href='https://webgescof.learneo.fr/formations/page.php?param=" . htmlspecialchars($data[1]) . "'>" . htmlspecialchars($data[1]) . "</a></td>";

                                echo "<td><a style='text-decoration:none;color:black;' href='https://webgescof.learneo.fr/formations/page.php?param=" . htmlspecialchars($data[1]) . "'>" . htmlspecialchars($data[2]) . "</a></td>";
                                $calcul = (int)($data[7])/7;
                                ...
                                // Ajouter des mois vides si nÃƒÂ©cessaire
                                for ($i = $mois; $i <= 12; $i++) {
                                    if (!isset($tab_mois[$i])) {
                                        $tab_mois[$i] = [];
                                    }
                                ...
                                foreach($tab_mois as $cle => $value){
                                    if($nb <= 5){
                                        ...
                                        if($value == []){
                                            ...
                                            //print_r($value);
                                            $nombre = 0; // Va servir ÃƒÂ  savoir s'il existe plusieurs dates dans le meme mois
                                            usort($value,"cmp");
                                            usort($value,"cmp_days"); // Sert ÃƒÂ  trier selon le jour 
                                            foreach($value as $date){
                                                ...
                                                $jour = explode("/",$date)[0];
                                                // Sert ÃƒÂ  ÃƒÂ©viter la rÃƒÂ©pÃƒÂ©tition + On fait la difference entre les deux dates.
            ...
            while ( ($data = fgetcsv($open_cle,2000,";")) == true && $i>=0  ){
                 ...
                 $i=$i+1;
                            ...
                            //Fonction qui sert ÃƒÂ  comparer les dates de debut dans notre dictionnaire
